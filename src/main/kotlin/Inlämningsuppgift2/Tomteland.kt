...
class Tomteland {
    ...
    val all = mapOf(
        "Tomten" to "Glader,Butter",
        "Glader" to "Tröger,Trötter,Blyger",
        "Trötter" to "Skumtomten",
        "Skumtomten" to "Dammråttan",
        "Butter" to "Räven,Haren,Rådjuret,Nyckelpigan",
        "Räven" to "Gråsuggan,Myran",
        "Myran" to "Bladlusen"
    )
    ...
    fun getUnderlings(currentName: String, res: MutableList<String>): List<String> {
        ...
        fun doRepeat(currentName: String){
            all[currentName]
                ?.split(",")
                ?.map {
                    res.add(it)
...
fun main() {
    ...
    println(t.getUnderlings("Glader", list))
}
